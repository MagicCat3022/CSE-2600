...
RESULTS_PATH = BASE_DIR / "rf_model_results.csv"
...
PARAM_GRID = {
    ...
    "regressor__max_depth": [50, None],
    "regressor__min_samples_split": [2, 5],
    "regressor__min_samples_leaf": [1, 3],
    "regressor__max_features": [50, 0.5],
}
...
def load_dataset() -> tuple[pd.DataFrame, pd.Series]:
    ...
    feature_cols = [col for col in data.columns if col != TARGET and col.lower() not in EXCLUDED_FEATURES]
    if not feature_cols:
        raise ValueError("No features remain after exclusions.")

    excluded_cols = [col for col in data.columns if col.lower() in EXCLUDED_FEATURES and col != TARGET]
...
def append_partial_result(row: dict, filepath: Path, header_written: bool) -> bool:
    ...
    for col in CLEAN_PARAM_NAMES:
        if col in row:
            ordered_row[col] = row[col]
    pd.DataFrame([ordered_row]).to_csv(filepath, mode="a", header=not header_written, index=False)
    return True
...
def run_hyperparameter_search(
    ...
    for idx, params in enumerate(param_grid, start=1):
        ...
        model_filename = f"rf_reg_{idx}.joblib"
        ...
        print(f"[{idx:02d}/{total_configs}] RMSE: {rmse_scores.mean():.3f}, params: {clean_params}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Random Forest Regression trainer.")
    parser.add_argument("--n-features", type=int, default=0, help="Reserved for parity; RF uses all features by default.")
    ...
    dst_model_path = best_models_dir / "best_model_rf.joblib"
